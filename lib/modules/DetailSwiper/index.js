...
var DetailSwiper = function DetailSwiper(props) {

  var map = props.__map__;
  var mapStore = props.__ele__;

  var render = props.render; // 底部 card 的渲染样式
  var loop = props.loop || true; // card 是否支持轮播
  var autoPlay = props.autoPlay || false; // 是否支持自动播放
  var arrowRender = props.arrowRender; // 箭头的渲染样式
  var animationEndHanler = props.animationEndHanler; // 底部 `card` 每次滑动效果结束触发的事件
  ...
  var getScope = function getScope(arg) {
    scope = arg;
  };

  var markerGoCenter = function markerGoCenter() {
    setTimeout(function () {
      var children = getChildren();
      var index = scope.state.index;
  ...
  var arrowHandler = function arrowHandler(dir) {
    if (dir === 'left') {
      scope.pre();
      markerGoCenter();
    } else {
  ...
  if (!map) {
    ...
    if (!window.changeSwiperLabel) {
      window.changeSwiperLabel = function () {
        setTimeout(function () {
  ...
  return (0, _rax.createElement)(
    ...
    (0, _rax.createElement)(
      _swiper2.default,
      { ref: 'swiper', autoPlay: autoPlay, loop: loop, width: 1, previewWidth: 298, getScope: getScope,
        animationEndHanler: animationEndHanler },
      getChildren().map(function (item) {
        console.log('ok=>', item.getExtData());
        return render instanceof Function ? render() : (0, _rax.createElement)(
          _raxView2.default,
          { style: itemStyle },
    ...
    (0, _rax.createElement)(
      _raxView2.default,
      { style: arrowStyle },
      arrowRender instanceof Function ? arrowRender('left') : (0, _rax.createElement)(
        _raxView2.default,
        { style: { justifyContent: 'center' }, onClick: arrowHandler.bind(undefined, 'left') },
        (0, _rax.createElement)(
          _raxView2.default,
          { style: arrowStyleL },
          ' ',
      ...
      arrowRender instanceof Function ? arrowRender('right') : (0, _rax.createElement)(
        _raxView2.default,
        { style: { justifyContent: 'center' }, onClick: arrowHandler.bind(undefined, 'left') },
        (0, _rax.createElement)(
          _raxView2.default,
